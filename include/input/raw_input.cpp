...
void getInputReportInfo(InputReportInfo* info) {
    ...
    if (pdataSize > 0) {
        ...
        info->pLinkCollection =
            (HIDP_LINK_COLLECTION_NODE*)malloc(sizeof(HIDP_LINK_COLLECTION_NODE) * linkNodeNumber);
...
void __getTouchPadInfoFile(InputReportInfo* info) {
    ...
    fprintf(file,
            ...
            pCaps->Usage, pCaps->UsagePage, pCaps->InputReportByteLength, pCaps->NumberLinkCollectionNodes,
            pCaps->NumberInputButtonCaps, pCaps->NumberInputValueCaps, pCaps->NumberInputDataIndices);
    ...
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "0x%-10X", pLinkCollection[i].LinkUsage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkUsagePage");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "0x%-10X", pLinkCollection[i].LinkUsagePage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Parent");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "%-12d", pLinkCollection[i].Parent);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NumberOfChildren");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "%-12d", pLinkCollection[i].NumberOfChildren);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NextSibling");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "%-12d", pLinkCollection[i].NextSibling);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "FirstChild");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "%-12d", pLinkCollection[i].FirstChild);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "CollectionType");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "0x%-10X", pLinkCollection[i].CollectionType);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsAlias");
    for (int i = 0; i < numLinkCollection; i++)
        fprintf(file, "%-12s", pLinkCollection[i].IsAlias ? "TRUE" : "FALSE");
    fprintf(file, "\n\n");
    ...
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].UsagePage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "ReportID");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].ReportID);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsAlias");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].IsAlias ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "BitField");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].BitField);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkCollection");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].LinkCollection);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkUsage");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].LinkUsage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkUsagePage");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].LinkUsagePage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsRange");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].IsRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsStringRange");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].IsStringRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsDesignatorRange");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].IsDesignatorRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsAbsolute");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].IsAbsolute ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "HasNull");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12s", pValueCaps[i].HasNull ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "BitSize");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].BitSize);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "ReportCount");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].ReportCount);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "UnitsExp");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12lu", pValueCaps[i].UnitsExp);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Units");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10lX", pValueCaps[i].Units);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LogicalMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12ld", pValueCaps[i].LogicalMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LogicalMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12ld", pValueCaps[i].LogicalMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "PhysicalMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12ld", pValueCaps[i].PhysicalMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "PhysicalMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12ld", pValueCaps[i].PhysicalMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.UsageMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].Range.UsageMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.UsageMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].Range.UsageMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.StringMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.StringMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.StringMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.StringMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DesignatorMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.DesignatorMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DesignatorMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.DesignatorMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DataIndexMin");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.DataIndexMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DataIndexMax");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].Range.DataIndexMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.Usage");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "0x%-10X", pValueCaps[i].NotRange.Usage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.StringIndex");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].NotRange.StringIndex);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.DesignatorIndex");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].NotRange.DesignatorIndex);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.DataIndex");
    for (int i = 0; i < numValCaps; i++)
        fprintf(file, "%-12d", pValueCaps[i].NotRange.DataIndex);
    fprintf(file, "\n\n");
    ...
    fprintf(file, "%-25s", "UsagePage");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].UsagePage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "ReportID");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].ReportID);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsAlias");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12s", pButtonCaps[i].IsAlias ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "BitField");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].BitField);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkCollection");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].LinkCollection);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkUsage");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].LinkUsage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "LinkUsagePage");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].LinkUsagePage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsRange");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12s", pButtonCaps[i].IsRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsStringRange");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12s", pButtonCaps[i].IsStringRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsDesignatorRange");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12s", pButtonCaps[i].IsDesignatorRange ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "IsAbsolute");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12s", pButtonCaps[i].IsAbsolute ? "TRUE" : "FALSE");
    fprintf(file, "\n");

    fprintf(file, "%-25s", "ReportCount");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].ReportCount);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.UsageMin");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].Range.UsageMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.UsageMax");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].Range.UsageMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.StringMin");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.StringMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.StringMax");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.StringMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DesignatorMin");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.DesignatorMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DesignatorMax");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.DesignatorMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DataIndexMin");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.DataIndexMin);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "Range.DataIndexMax");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].Range.DataIndexMax);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.Usage");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "0x%-10X", pButtonCaps[i].NotRange.Usage);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.StringIndex");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].NotRange.StringIndex);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.DesignatorIndex");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].NotRange.DesignatorIndex);
    fprintf(file, "\n");

    fprintf(file, "%-25s", "NotRange.DataIndex");
    for (int i = 0; i < numButtonCaps; i++)
        fprintf(file, "%-12d", pButtonCaps[i].NotRange.DataIndex);
    fprintf(file, "\n\n");
    fclose(file);
}
